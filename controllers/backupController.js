...
const processCursor = async (cursor, tablename, type, columnpk, client) => {
    let indexPart = 1;
    ...
    return new Promise((resolve, reject) => {
        (function read() {
            cursor.read(BATCH_SIZE, async (err, rows) => {
                ...
                const query = toQuery(rows, tablename, type, columnpk, client);
...
exports.incremental = async (req, res) => {
    ...
    try {
        ...
        for (const table of tablesToBackup) {
            ...
            await processCursor(cursor, tablename, type, columnpk, client)
...
const toQuery = (data, tablename, type = "NINGUNO", columnpk, client) => {
