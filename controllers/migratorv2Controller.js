...
exports.executeMigration = async (req, res) => {
    ...
    if (!!corpid && !!modules) {
        ...
        await zyxmeQuery(`ALTER TABLE migration ADD COLUMN IF NOT EXISTS orgid bigint`, bind = {
            _requestid: req._requestid,
        });
        let migrationstatus = await zyxmeQuery(`SELECT corpid, orgid FROM migration WHERE corpid = $corpid and orgid = $orgid`, bind = {
            _requestid: req._requestid,
            corpid: corpid,
            orgid: orgid,
        });
