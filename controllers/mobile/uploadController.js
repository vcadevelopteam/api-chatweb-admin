...
exports.upload = async (req, res) => {
    ...
    try {
        console.log("uploadfile " + id, new Date() + " start upload");
        ...
        s3.upload(params, (err, data) => {
            ...
            console.log("uploadfile " + id, new Date() + " upload success");
            return res.json({ success: true, url: data.Location })
        })   
    } 
    catch (error) {
        console.log("error try", error);
        return res.json({ success: false, msg: 'Hubo un error#2 en la carga de archivo.', error })
    }
}