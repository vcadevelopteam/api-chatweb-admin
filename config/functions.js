module.exports = {
    ...
    UFN_REPORT_INTERACTION_SEL: {
        query: "WITH w1 AS ( 	SELECT DISTINCT unnest(string_to_array(groups,',')) AS groups 	FROM orguser ous 	WHERE ous.corpid = $corpid 	AND ous.orgid = $orgid 	AND ous.userid = $userid ) SELECT  co.ticketnum, to_char(co.startdate + $offset * INTERVAL '1hour', 'YYYY') as ticketyear, to_char(co.startdate + $offset * INTERVAL '1hour', 'MM') as ticketmonth, co.startdate as ticketdatehour, inter.interactionid, inter.createdate as interactiondatehour, REGEXP_REPLACE(pe.name, E'[\\n\\r\\t]+',' ','g') as person, pcc.displayname as originalname, cc.description channel, concat(us.firstname,' ',us.lastname) as agent, inter.intent, inter.interactiontype, REGEXP_REPLACE(inter.interactiontext, E'[\\n\\r\\t]+',' ','g') as interactiontext, pe.phone as clientnumber, co.personcommunicationchannel FROM conversation co LEFT JOIN orguser ous ON co.corpid = ous.corpid AND co.orgid = ous.orgid AND co.lastuserid = ous.userid LEFT JOIN communicationchannel cc on co.corpid = cc.corpid and co.orgid = cc.orgid and co.communicationchannelid = cc.communicationchannelid LEFT JOIN person pe on co.corpid = pe.corpid and co.orgid = pe.orgid and co.personid = pe.personid LEFT JOIN personcommunicationchannel pcc on co.corpid = pcc.corpid and co.orgid = pcc.orgid and co.personid = pcc.personid and co.personcommunicationchannel = pcc.personcommunicationchannel LEFT JOIN interaction inter ON co.conversationid = inter.conversationid LEFT JOIN orguser ous2 ON inter.corpid = ous2.corpid AND inter.orgid = ous2.orgid AND inter.userid = ous2.userid LEFT JOIN usr us on ous2.userid = us.userid WHERE co.corpid = $corpid AND co.orgid = $orgid and inter.createdate between $startdate::timestamp - $offset * INTERVAL '1hour' and $enddate::timestamp + interval '1 day' - $offset * INTERVAL '1hour' AND CASE WHEN (SELECT(array_length(array_agg(groups), 1)) FROM w1) IS NOT NULL THEN (string_to_array(ous2.groups,',') && (SELECT array_agg(groups) FROM w1)) OR COALESCE(ous2.groups,'') = '' ELSE TRUE END AND inter.interactiontype <> 'LOG' ###WHERE### ###ORDER### LIMIT $take  OFFSET $skip;",
        module: "/reports",
        protected: "SELECT"
    },
    UFN_REPORT_INTERACTION_TOTALRECORDS: {
        query: "WITH w1 AS (     SELECT DISTINCT unnest(string_to_array(groups,',')) AS groups     FROM orguser ous     WHERE ous.corpid = $corpid     AND ous.orgid = $orgid     AND ous.userid = $userid ) SELECT COUNT(*) as p_totalrecords FROM conversation co LEFT JOIN orguser ous ON co.corpid = ous.corpid AND co.orgid = ous.orgid AND co.lastuserid = ous.userid LEFT JOIN communicationchannel cc on co.corpid = cc.corpid and co.orgid = cc.orgid and co.communicationchannelid = cc.communicationchannelid LEFT JOIN person pe on co.corpid = pe.corpid and co.orgid = pe.orgid and co.personid = pe.personid LEFT JOIN personcommunicationchannel pcc on co.corpid = pcc.corpid and co.orgid = pcc.orgid and co.personid = pcc.personid and co.personcommunicationchannel = pcc.personcommunicationchannel LEFT JOIN interaction inter ON co.conversationid = inter.conversationid LEFT JOIN orguser ous2 ON inter.corpid = ous2.corpid AND inter.orgid = ous2.orgid AND inter.userid = ous2.userid LEFT JOIN usr us on ous2.userid = us.userid WHERE co.corpid = $corpid AND co.orgid = $orgid AND inter.createdate between $startdate::timestamp - $offset * INTERVAL '1hour' and $enddate::timestamp + interval '1 day' - $offset * INTERVAL '1hour' and CASE WHEN (SELECT(array_length(array_agg(groups), 1)) FROM w1) IS NOT NULL THEN (string_to_array(ous2.groups,',') && (SELECT array_agg(groups) FROM w1)) OR COALESCE(ous2.groups,'') = '' ELSE TRUE END AND inter.interactiontype <> 'LOG' ###WHERE###;",
        module: "/reports",
        protected: "SELECT"
    },
    ...
    UFN_ORG_INS: {
        query: "SELECT * FROM ufn_org_ins($corpid,$id,$description,$status,$type,$username,$operation,$email,$password,$port,$host,$default_credentials,$ssl, $private_mail, $currency,$country, $timezoneoffset, $timezone, $doctype, $docnum, $businessname, $fiscaladdress, $sunatcountry, $contactemail, $contact, $autosendinvoice)",
        module: "/extras/quickreplies",
        protected: "INSERT"
    },
    UFN_QUICKREPLY_INS: {
    ...
    UFN_LEADACTIVITY_INS: {
        query: "select * from ufn_leadactivity_ins($corpid,$orgid,$leadid,$leadactivityid,$description,$duedate,$assignto,$type,$status,$username,$operation, $feedback, $detailjson)",
        module: "",
        protected: "INSERT"
    },
